...
def test_html_unescape(df):
    # Compiling a regex pattern to find HTML entities
    html_entity_re = re.compile(r"&[A-Za-z0-9#]+;")
    # Ensure there are no HTML entities in the text
    assert all(
...
def test_no_extra_spaces(df: pd.DataFrame):
    assert all(
        "  " not in x for x in df["job_description"]
    ), "Extra spaces found in the data"
    return df

def test_no_accents(df: pd.DataFrame):
    for desc in df['job_description']:
        if unicodedata.normalize('NFKD', desc) != "".join([c for c in unicodedata.normalize('NFKD', desc) if not unicodedata.combining(c)]):
            print(f"Accent found in: {desc}")
            break
    assert all(
        unicodedata.normalize('NFKD', x) == "".join([c for c in unicodedata.normalize('NFKD', x) if not unicodedata.combining(c)])
        for x in df["job_description"]
    ), "Accents found in the data"
    return df

def test_no_currency_symbols(df: pd.DataFrame):
    currency_symbols_pattern = r'[\$\€\£\¥\₹\₩\₽]'
    for desc in df['job_description']:
        if re.sub(currency_symbols_pattern, '', desc) != desc:
            print(f"Currency symbol found in: {desc}")
            break
    assert all(
        re.sub(currency_symbols_pattern, '', x) == x for x in df["job_description"]
    ), "Currency symbols found in the data"
    return df
